...
function getStepFootprintLength(params, stepCount, stepLength) {
  ...
  if (isCenteredCircular) return narrowRun;
...
function createStepGeometry(runLength, stepWidth, height, params, layout) {
  ...
  if (shape === "rectangle" || (isFullWidthStep && !isCenteredCircularStep)) {
...
floor.userData.type = "floor";
  ...
  if (stepCount > 0) {
    ...
    for (let s = 0; s < stepCount; s++) {
      ...
      const isCenteredCircular = params?.stepShape === "circular" && layout.position === "center";
      const centeredCircularRadius = Math.max(0.05, Number(params?.stepExtension) || ((Number(params?.stepWidth) || STEP_PRESET_WIDTH) * 0.5));
      const centeredCircularStepCount = Math.max(1, stepCount);
      const centeredCircularRun = centeredCircularRadius * ((s + 1) / centeredCircularStepCount);
      const isDiagonalNarrow = (params?.stepShape === "diagonal" || params?.stepShape === "circular") && s !== 1 && layout.position !== "center";
      const stepRun = isCenteredCircular ? centeredCircularRun : (isDiagonalNarrow ? diagonalStepSize : (s === 1 ? bench2Extension : STEP_LENGTH));
      const stepWidthForGeo = isCenteredCircular ? centeredCircularRun * 2 : (isDiagonalNarrow ? diagonalStepSize : layout.width);
