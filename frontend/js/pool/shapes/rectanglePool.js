  ...
  if (stepCount > 0) {
    ...
    for (let s = 0; s < stepCount; s++) {
      ...
      if (stepBenchMode === "stepsOnly") {
        const nestedFullWidth = Math.max(0.05, width);
        ...
        let nestedCenterY = 0;
        if (narrowLayout.position === "left") nestedCenterY = -width * 0.5 + nestedWidth * 0.5;
        if (narrowLayout.position === "right") nestedCenterY = width * 0.5 - nestedWidth * 0.5;
