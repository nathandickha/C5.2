...
floor.userData.type = "floor";
  ...
  if (stepCount > 0) {
    ...
    for (let s = 0; s < stepCount; s++) {
      ...
      const layout = isBenchSeat ? { ...fullStepLayout, isBenchSeat: true } : baseLayout;
      ...
      const stepRun = isBenchSeat ? bench2Extension : (isCenteredCircular ? centeredCircularRun : (isDiagonalNarrow ? diagonalStepSize : STEP_LENGTH));
      const stepWidthForGeo = isBenchSeat ? layout.width : (isCenteredCircular ? centeredCircularRun * 2 : (isDiagonalNarrow ? diagonalStepSize : layout.width));
      ...
      const x = isCenteredCircular
        ? (-length / 2 + (stepBenchMode === "bench" && stepCount > 1 && s > 1 ? bench2Extension : 0) + stepRun * 0.5)
        : (stepBenchMode === "stepsOnly"
            ? -length / 2 + (s * stepRun) + stepRun * 0.5
            : (s <= 1
