...
floor.userData.type = "floor";
  ...
  if (stepCount > 0) {
    ...
    const fullWidth = Math.max(0.05, width);
    const getNestedLayout = (stepIndex) => {
      const targetWidth = Math.min(fullWidth, baseWidth * (stepIndex + 1));
      let centerY = 0;
      if (narrowLayout.position === "left") centerY = -width * 0.5 + targetWidth * 0.5;
      if (narrowLayout.position === "right") centerY = width * 0.5 - targetWidth * 0.5;
