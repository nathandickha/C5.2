...
function getStepFootprintLength(params, stepCount, stepLength) {
  ...
  // Bench Seat mode keeps the old full-width second bench. Steps Only mode
  // removes that bench for every step shape and chains normal steps only.
  if (isCenteredCircular) {
    return stepBenchMode === "bench" && count > 1
      ? bench2Extension + centeredCircularRun
      : centeredCircularRun;
  }
  if (stepBenchMode === "stepsOnly") return count * narrowRun;
  ...
  if (stepCount > 0) {
    ...
    for (let s = 0; s < stepCount; s++) {
      ...
      if (stepBenchMode === "stepsOnly") {
        const nestedFullWidth = Math.max(0.05, wallMaxY - wallMinY);
        const nestedBaseWidth = Math.max(0.05, Number(narrowLayout.width) || STEP_PRESET_WIDTH);
        const nestedWidth = Math.min(nestedFullWidth, nestedBaseWidth * (s + 1));
        let nestedCenterY = (wallMinY + wallMaxY) * 0.5;
        if (narrowLayout.position === "left") nestedCenterY = wallMinY + nestedWidth * 0.5;
        if (narrowLayout.position === "right") nestedCenterY = wallMaxY - nestedWidth * 0.5;
      ...
      let stepRun = isBenchSeat ? bench2Extension : (isCenteredCircular ? centeredCircularRun : (isDiagonalNarrow ? diagonalStepSize : STEP_LENGTH));
        ...
        if (stepBenchMode === "stepsOnly") {
          if (isCenteredCircular) {
            ...
            stepRun = Math.max(0.05, diagonalStepSize * (s + 1));
            stepWidthForGeo = stepRun;
          } else {
            stepRun = Math.max(0.05, STEP_LENGTH * (s + 1));
            stepWidthForGeo = layout.width;
          }
        }
      ...
      const x = isCenteredCircular
        ? (wallMinX + (stepBenchMode === "bench" && stepCount > 1 && s > 1 ? bench2Extension : 0) + stepRun * 0.5)
