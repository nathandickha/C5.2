...
function createStepGeometry(runLength, stepWidth, height, params, layout) {
  ...
  if (pos === "center") {
    if (shape === "circular") {
      // Centre circular steps use a half-round footprint: straight along the
      // wall/back edge, with a rounded front like a semi-circle. We fit that
      // profile inside the current run/width controls so the existing sliders
      // continue to work without changing the rest of the step layout logic.
      const run = Math.max(0.05, Number(runLength) || 0.05);
      const width = Math.max(0.05, Number(stepWidth) || 0.05);
      const x0 = -run * 0.5;
      const y0 = -width * 0.5;
