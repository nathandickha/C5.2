...
function getStepFootprintLength(params, stepCount, stepLength) {
  ...
  const centeredCircularMode = params?.centerCircularStepMode === "bench" ? "bench" : "stepsOnly";
  ...
  if (isCenteredCircular) {
    return centeredCircularMode === "bench" && count > 1
      ? bench2Extension + centeredCircularRun
      : centeredCircularRun;
  }
...
/* =====================================================
    ...
    if (stepCount > 0) {
      ...
      for (let s = 0; s < stepCount; s++) {
        ...
        const baseLayout = s === 1 ? fullStepLayout : narrowLayout;
        const isCenteredCircular = liveParams?.stepShape === "circular" && baseLayout.position === "center";
        const centeredCircularMode = liveParams?.centerCircularStepMode === "bench" ? "bench" : "stepsOnly";
        const isCenterBenchSeat = isCenteredCircular && centeredCircularMode === "bench" && s === 1 && stepCount > 1;
        const layout = isCenterBenchSeat ? { ...fullStepLayout, isBenchSeat: true } : baseLayout;
        ...
        const centeredCircularOrdinal = centeredCircularMode === "bench" && stepCount > 1
          ? (s < 1 ? s + 1 : s)
          : (s + 1);
        const centeredCircularRun = centeredCircularRadius * (centeredCircularOrdinal / centeredCircularStepCount);
        const isDiagonalNarrow = (liveParams?.stepShape === "diagonal" || liveParams?.stepShape === "circular") && s !== 1 && layout.position !== "center";
        const stepRun = isCenterBenchSeat ? bench2Extension : (isCenteredCircular ? centeredCircularRun : (isDiagonalNarrow ? diagonalStepSize : (s === 1 ? bench2Extension : STEP_LENGTH)));
        const stepWidthForGeo = isCenterBenchSeat ? layout.width : (isCenteredCircular ? centeredCircularRun * 2 : (isDiagonalNarrow ? diagonalStepSize : layout.width));
        ...
        const x = isCenteredCircular
          ? (axisStartWallX + (centeredCircularMode === "bench" && stepCount > 1 && s > 1 ? bench2Extension : 0) + stepRun * 0.5)
          : (s === 0
