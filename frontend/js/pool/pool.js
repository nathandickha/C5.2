...
/* =====================================================
    ...
    if (stepCount > 0) {
      ...
      for (let s = 0; s < stepCount; s++) {
        ...
        const shapeName = (["diagonal", "circular", "radius"].includes(liveParams?.stepShape)) ? liveParams.stepShape : "rectangle";
        const canCurveFollow = shapeName === "rectangle" || shapeName === "radius";
        const stepMinY = layout.centerY - stepWidthForGeo * 0.5;
        const stepMaxY = layout.centerY + stepWidthForGeo * 0.5;
        const curvedGeo = canCurveFollow
          ? createCurveFollowStepGeometry(perimeter2D, axisStartWallX, stepRun, stepMinY, stepMaxY, h, liveParams, layout)
          : null;
        const geo = curvedGeo || createStepGeometry(stepRun, stepWidthForGeo, h, liveParams, layout);
        const step = new THREE.Mesh(geo, new THREE.MeshStandardMaterial());

        const x = curvedGeo ? 0 : (s === 0
        ...
        step.position.set(x, curvedGeo ? 0 : layout.centerY, z);
