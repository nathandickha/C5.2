...
export class PoolApp {
    ...
    this.poolParams = {
      ...
      stepPosition: "center",
      stepShape: "rectangle",
      centerCircularStepMode: "stepsOnly",
...
// --------------------------------------------------------------
  ...
  getCenterCircularStepMode() {
    return this.poolParams?.centerCircularStepMode === "bench" ? "bench" : "stepsOnly";
  }

  updateCenterCircularModeControls() {
    const row = document.getElementById("centerCircularModeControls");
    if (row) row.style.display = this.isCenteredCircularStepShape?.() ? "block" : "none";
    const mode = this.getCenterCircularStepMode?.() ?? "stepsOnly";
    document.querySelectorAll("[data-center-circular-mode]").forEach((btn) => {
      btn.classList.toggle("active", btn.dataset.centerCircularMode === mode);
    });
  }
  ...
  setupStepLayoutControls() {
    ...
    document.querySelectorAll("[data-step-shape]").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (next === "diagonal" || (next === "circular" && this.poolParams.stepPosition !== "center")) {
          ...
          if (!this.poolParams.centerCircularStepMode) this.poolParams.centerCircularStepMode = "stepsOnly";
    ...
    document.querySelectorAll("[data-center-circular-mode]").forEach((btn) => {
      btn.addEventListener("click", () => {
        const next = btn.dataset.centerCircularMode === "bench" ? "bench" : "stepsOnly";
        if (this.poolParams.centerCircularStepMode === next) return;
        this.captureUndoState?.("Centre circular layout");
        this.poolParams.centerCircularStepMode = next;
        this.updateCenterCircularModeControls?.();
        rebuildForStepLayout("Centre circular layout");
      });
    });
